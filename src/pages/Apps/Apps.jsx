...
const Apps = () => {
  ...
  const filterdApp = appData.filter((app) =>
    ...
    <div className="px-10 mb-5">
      <div className="text-center py-10 max-w-3xl mx-auto">
        ...
        <p className="text-gray-500">
          All your apps, now in one place. All apps in our section are neatly
          organized and instantly ready to use. From work to fun, manage and
          launch your favorites with ease.
        </p>
      </div>
      <div className="max-w-360 mx-auto md:flex justify-between items-center px-3 space-y-5 text-center mb-5">
        <p className="text-2xl font-semibold">
          ({filterdApp.length}) Apps Found
        </p>
        <label className="input">
          ...
          <input
            ...
            type="search"
            required
            placeholder="Search by title or 'chat', 'video', 'streaming' "
          />
        </label>
      </div>
      ...
      {!filterdApp.length <= 0 ? (
        <div className="max-w-360 mx-auto grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-6 p-3">
          {filterdApp.map((app) => (
            <Link key={app?.id} to={`/appDetails/${app.id}`}>
              <div
                className=" bg-base-100 shadow-sm rounded flex flex-col h-full
