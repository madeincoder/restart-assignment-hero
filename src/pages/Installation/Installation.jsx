...
const Installation = () => {
  ...
  const handleLowToHeigh = () => {
    const filtered = [...appsR].sort((p, c) => p.downloads - c.downloads);
    setSortApp(filtered);
  };

  const handleHeighToLow = () => {
    const filtered = [...appsR].sort((p, c) => c.downloads - p.downloads);
    setSortApp(filtered);
  };

  const shorted = sortApp || appsR;

  return (
    <div className="bg-gray-100">
      <div className="max-w-360 mx-auto w-full p-10">
        <div className="text-center py-10 max-w-3xl mx-auto">
          <h2 className="text-5xl  font-bold mb-5 mt-10">My Installed Apps</h2>
          <p className="text-gray-500">
            View all your installed apps in one place and manage them
            effortlessly. Also installed apps, ready to open, update, or manage
            anytime.
          </p>
        </div>
        <div className="md:flex justify-between items-center  space-y-5 text-center mb-5">
          <p className="text-2xl font-semibold">{appsR.length} Apps Found</p>
          <div className="dropdown dropdown-end">
            ...
            >
              <li>
                <a onClick={handleHeighToLow}>High-Low</a>
              </li>
              <li>
                <a onClick={handleLowToHeigh}>Low-Heigh</a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          {shorted.length !== 0 || shorted.length < 0 ? (
            shorted.map((app) => (
              <div
                key={app?.id}
                className="bg-white rounded p-5 flex justify-between items-center "
              >
                  ...
                  <div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <p className="flex items-center gap-1 text-yellow-500">
                        <IoStar /> <span>{app?.ratingAvg}</span>
                      </p>
